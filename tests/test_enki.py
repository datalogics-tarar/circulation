...
class TestEnkiAPI(DatabaseTest, BaseEnkiTest):
    ...
    def setup(self):
        ...
        self.api = MockEnkiAPI(self.collection,self._db)
...
class TestBibliographicCoverageProvider(TestEnkiAPI):
    ...
    def test_script_instantiation(self):
        ...
        script = RunCollectionCoverageProviderScript(
            ...
            api_class=MockEnkiAPI
        )
        [provider] = script.providers
        assert isinstance(provider,
                          EnkiBibliographicCoverageProvider)
        assert isinstance(provider.api, MockEnkiAPI)
    ...
    def test_process_item_creates_presentation_ready_work(self):
        ...
        provider = EnkiBibliographicCoverageProvider(
            self.collection, self._db, enki_api=self.api
        )
